...
- 在一个中大型项目中，随着时间的增长，一些组件的代码和逻辑会越来越复杂。无法短时间内理清项目逻辑，导致上手速度慢，成本高
...
2. 测试思路：它不在意组件实现细节，而是聚焦在了组件能力本身(即从developer传入props，然后通过RTL的API验证`render`函数输出的内容),从用户视角进行测试，如下，用户只会看到组件渲染后是否有`learn react`这一串字符：
```js
