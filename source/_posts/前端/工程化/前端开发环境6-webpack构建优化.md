...
在根目录创建一个 dll 目录，然后运行`webpack --config webpack.dll.js`，将一些基础包提前编译出来，dll 目录下会有 manifest.json react.dll.js 两个文件
...
然后将这些排除的第三方库的稳定包采用 cdn 部署，并在项目的入口`index.html`文件中引入
