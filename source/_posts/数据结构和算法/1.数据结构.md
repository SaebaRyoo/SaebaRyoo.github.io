...
这是一种最简单，最基础的数据结构，在js中是原生支持的

## 栈

在js中，栈和队列都没有原生的实现，需要依赖数组实现


栈是后进先出的一个数据结构，所以只能用到pop和push完成增删
```js
class Stack {
    constructor() {
        this.arr = [];
...
队列是先进先出的数据结构,所以只要用push和shift完成增删
```js
class Queue {
    constructor() {
        this.arr = [];
...
<div align="center">
   <img src="https://i.postimg.cc/RZVtFCYB/image.png" width = "300" alt="" align=center />
</div>

每一个结点的结构都包含了两部分内容：**数据域**和**指针域**。js中的链表，是以嵌套的对象的形式来实现的。
...
用图片表示为
<div align="center">
   <img src="https://i.postimg.cc/D0XgxfTD/image.png" width = "300" alt="" align=center />
</div>
...
在生成链表数据时，需要传入val(数据域对应的值)、指定next（下一个链表结点）
...
在定义二叉树构造函数时，需要将左侧子结点和右侧子结点都预置为null
